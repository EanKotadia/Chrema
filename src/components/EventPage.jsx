  ...
  return (
    <div className="event-form">
      ...
      <div className="ef-field">
        ...
        <input className="ef-input" value={form.title} onChange={set("title")} placeholder={form.type === "drawing" ? "Name your artwork" : "Article title"} />
      </div>
      ...
      >
        {uploading ? "Uploading image…" : submitting ? "Submitting…" : `Submit ${form.type === "drawing" ? "Artwork" : "Article"} →`}
      </button>
    </div>
  );
}
...
export default function EventPage({ slug }) {
  ...
  const [tab, setTab]               = useState("about"); // about | submit | gallery
  ...
  useEffect(() => {
    Promise.all([
      getEventBySlug(slug),
      getEventSubmissions(null), // will filter by slug
    ]).then(([ev, subs]) => {
  ...
  if (loading) return <div className="loading-state" style={{minHeight:"100vh"}}><div className="loading-spinner"/></div>;
  if (notFound) return (
  ...
  const isOpen    = event.status === "active" && (!event.deadline || new Date(event.deadline) > Date.now());
    ...
    <div className="event-page" style={{"--ev-accent": accent}}>
        ...
        <div className="event-banner-inner">
          <div className="event-banner-left">
            ...
            <div className="event-meta-pills">
              ...
              {isArchived && <span className="event-pill event-pill--archived">Closed</span>}
            </div>
          </div>
          <div className="event-banner-right">
            {isOpen && event.deadline && new Date(event.deadline) > Date.now() && (
            ...
            {!isOpen && !isArchived && (
              <div className="event-countdown-wrap">
                <span className="event-countdown-label">Status</span>
                <div className="cd-expired">Submissions closed</div>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="event-tabs">
        {[
          ...
          { key: "submit",  label: isOpen ? "Submit Entry" : "Submissions Closed", disabled: !isOpen },
          { key: "gallery", label: `Gallery${approvedCount ? ` (${approvedCount})` : ""}` },
          ...
          <div className="event-about">
            <div className="event-about-inner">
              ...
              <div className="event-about-side">
                {isOpen && event.deadline && (
                  <div className="event-side-card">
                    <span className="ev-section-label">Deadline</span>
                    <p className="event-deadline-text">
                      {(() => {
        ...
        {tab === "gallery" && (
          <div className="event-gallery-wrap">
            <div className="event-gallery-header">
              ...
              <p>{approvedCount ? `${approvedCount} published ${approvedCount === 1 ? "entry" : "entries"}` : "No approved submissions yet."}</p>
            </div>
