  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        ...
        const cleanupScroll = trackScrollDepth(art.id);
        const cleanupTime   = trackTimeOnPage(art.id);
        ...
        // return cleanup so scroll/time listeners are removed on unmount
    ...
    <div className="article-page">
      ...
      <article className="article-body-wrap">
        ...
        <div className="article-header">
          {article.category && <span className="article-category-tag">{article.category}</span>}
          ...
          <div className="article-byline">
            <div className="byline-left">
              ...
              <div className="byline-info">
                {article.author && <span className="byline-name">By {article.author}</span>}
                {(article.published_at || article.created_at) && (
                  <span className="byline-date">{formatDate(article.published_at || article.created_at)}</span>
                )}
              </div>
            </div>
        ...
        <div className="article-content">
          {paragraphs.length > 0 ? (
            paragraphs.map((para, i) => {
              ...
              return isHtml
                ? <div key={i} className={i === 0 ? "article-first-para" : "article-html-block"}
                ...
                <a key={rel.id} href={`/article/${rel.id}`} className="related-card">
                  ...
                  <div className="related-info">
                    ...
                    <span className="related-meta">
                      By {rel.author}{rel.published_at && ` · ${formatDate(rel.published_at)}`}
                    </span>
                  </div>
                </a>
              ))}
