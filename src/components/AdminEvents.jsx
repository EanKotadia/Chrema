...
// Format any ISO string for display in IST
...
// Convert a stored UTC ISO string → "YYYY-MM-DDTHH:mm" in IST
// so the datetime-local input shows the correct local time.
...
function EventForm({ initial, onSave, onCancel }) {
  const normalizeForForm = (ev) => {
    ...
    return {
      ...ev,
      start_date: isoToISTLocal(ev.start_date),
      deadline:   isoToISTLocal(ev.deadline),
    };
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        // datetime-local is treated as IST — convert to UTC ISO for storage
    ...
    <div className="aev-form">
      ...
      <div className="aev-form-row">
        <div className="aev-field">
          ...
          <input className="aev-input" type="datetime-local" value={form.start_date} onChange={set("start_date")} />
        </div>
        <div className="aev-field">
          <label className="aev-label">
      ...
      {(form.start_date || form.deadline) && (
        <div className="aev-window-preview">
          <span className="aev-window-preview-label">Submission window (IST)</span>
          <span className="aev-window-preview-val">
            {form.start_date ? fmtDateTime(form.start_date) : "Now"}
            {" → "}
            {form.deadline ? fmtDateTime(form.deadline) : "No end date"}
          </span>
        </div>
      )}
...
function SubRow({ sub, onApprove, onReject, onNeedsEdit, onSaveEdit, onDelete }) {
  ...
  const [editForm, setEditForm] = useState({
    title: sub.title || "", body: sub.body || "", name: sub.name || "",
    grade: sub.grade || "", school: sub.school || "", admin_note: sub.admin_note || "",
  });
  const [saving, setSaving] = useState(false);
  ...
  const statusLabel = {
    pending: "Pending", approved: "Approved", rejected: "Rejected", needs_edit: "Needs Edit",
  ...
  return (
    <>
      <div className={`aev-sub-row ${open || editing ? "expanded" : ""}`}>
        ...
        <div className="aev-sub-date">
          {new Date(sub.submitted_at).toLocaleDateString("en-GB", { timeZone: IST })}
        </div>
        ...
        <div className="aev-sub-actions">
          ...
          >✎</button>
          {sub.status !== "approved"   && <button className="aev-action-btn aev-action-btn--approve"    onClick={() => onApprove(sub)}   title="Approve">✓</button>}
          {sub.status !== "rejected"   && <button className="aev-action-btn aev-action-btn--reject"     onClick={() => onReject(sub)}    title="Reject">✕</button>}
          {sub.status !== "needs_edit" && <button className="aev-action-btn aev-action-btn--needs-edit" onClick={() => onNeedsEdit(sub)} title="Needs Edit">⚑</button>}
      ...
      {open && !editing && (
        <div className="aev-sub-detail">
          ...
          <p className="aev-sub-email">Email: <a href={`mailto:${sub.email}`}>{sub.email}</a> · School: {sub.school || "—"}</p>
        </div>
      )}
        ...
        <div className="aev-sub-edit">
          ...
          {sub.type === "article" && (
            <div className="aev-edit-field">
              <label className="aev-label">
                Article Body
                <span className="aev-word-count">{editForm.body.trim().split(/\s+/).filter(Boolean).length} words</span>
              </label>
              <textarea className="aev-input aev-textarea aev-textarea--tall" value={editForm.body} onChange={setF("body")} rows={16} />
            </div>
          )}
          <div className="aev-edit-field">
            <label className="aev-label">
            ...
            <textarea className="aev-input aev-textarea" value={editForm.admin_note} onChange={setF("admin_note")} rows={3}
          ...
          <div className="aev-edit-actions">
            ...
            <button className="aev-btn aev-btn--needs-edit" onClick={() => { onNeedsEdit(sub); setEditing(false); }}>Mark as Needs Edit</button>
            <button className="aev-btn aev-btn--save" onClick={handleSave} disabled={saving}>{saving ? "Saving…" : "Save Changes"}</button>
          </div>
        </div>
      )}
    </>
...
export default function AdminEvents() {
  ...
  const loadSubs      = (ev) => { setActiveEvent(ev); getEventSubmissions(ev.id).then(data => setSubmissions(Array.isArray(data) ? data : [])); };
  ...
  const handleDelete  = async (ev) => { if (!window.confirm(`Delete "${ev.title}"?`)) return; await deleteEvent(ev.id); load(); showToast("Event deleted.", "error"); };
  const handleArchive = async (ev) => { await updateEvent(ev.id, { status: ev.status === "active" ? "archived" : "active" }); load(); showToast(ev.status === "active" ? "Event archived." : "Event re-activated."); };
  ...
  const handleDeleteSub = async (sub) => { if (!window.confirm("Permanently delete this submission?")) return; await deleteEventSubmission(sub.id); setSubmissions(p => p.filter(s => s.id !== sub.id)); showToast("Submission deleted.", "error"); };
  ...
  if (creating || editing) {
    return (
      <div className="aev-page">
        ...
        <EventForm initial={editing || null} onSave={editing ? handleEdit : handleCreate} onCancel={() => { setCreating(false); setEditing(null); }} />
      </div>
    );
  }
            ...
            return (
              <div key={ev.id} className="aev-event-row">
                ...
                <div className="aev-event-actions">
                  ...
                  <button className="aev-row-btn" onClick={() => handleArchive(ev)}>{ev.status === "active" ? "Archive" : "Activate"}</button>
